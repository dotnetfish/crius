...
import cn.cloudartisan.crius.client.exception.NetWorkDisableException;
import cn.cloudartisan.crius.client.exception.WriteToClosedSessionException;
import cn.cloudartisan.crius.client.filter.ClientMessageCodecFactory;
...
class CIMConnectorManager  {
	...
	public void deliverIsConnected() {
		...
		intent.putExtra(cn.cloudartisan.crius.client.android.CIMPushManager.KEY_CIM_CONNECTION_STATUS, isConnected());
		context.sendBroadcast(intent);
	}
	...
	IoHandlerAdapter iohandler = new IoHandlerAdapter() {
		...
		public void exceptionCaught(IoSession session, Throwable cause)
			...
			context.sendBroadcast(intent);
			Log.i(TAG, "******************CIM exceptionCaught:"+session.getLocalAddress());
		}
		...
		public void messageReceived(IoSession session, Object obj)
			...
			if (obj instanceof Message) {
				...
				//Log.e("connector message:",((Message) obj).getType());
				if(message.getType()==null){
					Log.e("connector message:"," message is null? "+obj);
					return;//未知错误，消息接受到两条。一条为空
				}
				Intent intent = new Intent();
