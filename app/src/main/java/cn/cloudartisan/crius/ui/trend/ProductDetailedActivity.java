...
public class ProductDetailedActivity extends CIMMonitorActivity implements AdapterView.OnItemClickListener, SimpleInputPanelView.OnOperationListener, HttpAPIResponser, CustomDialog.OnOperationListener {
    ...
    public void initComponents() {
        ...
        article = (ProductInfo) getIntent().getSerializableExtra(ProductInfo.class.getSimpleName());
        ...
        apiParams.put("articleId", article.getId());
    ...
    public void displayHeaderView() {
        ...
        if(article.getImgThumbs()!=null) {
            imageView.load(URLConstant.DOMAIN + article.getImgThumbs(), R.drawable.icon_head_default);
        }
        //(WebImageView) headerView.findViewById(R.id.icon);
       // JSONObject json = JSON.parseObject(article.getBrief());
        if (StringUtils.isNotEmpty(article.getBrief())) {
            headerView.findViewById(R.id.text).setVisibility(View.VISIBLE);

            ((TextView) headerView.findViewById(R.id.text)).setText(article.getBrief());
        }
        ((TextView) headerView.findViewById(R.id.name)).setText(article.getTitle());
        //((TextView) headerView.findViewById(R.id.time)).setText(AppTools.howTimeAgo(this,
         //       Long.valueOf(article.).longValue()));
        ((TextView) headerView.findViewById(R.id.time)).setText(article.getPublishTime());
       /* if (!StringUtils.isEmpty(article.())) {
            List<SNSImage> snsImageList = (List) JSON.parseObject(article.thumbnail, new TypeReference<ProductDetailedActivity>(){}.getType(), new Feature[0x0]);
    ...
    public void onSendContent(String content) {
        ...
        commentRequester.execute(new TypeReference<ProductDetailedActivity>(){}.getType(), null, URLConstant.COMMENT_PUBLISH_URL);
    ...
    public Map getRequestParams(String code) {
        apiParams.put("articleId", article.getId());
        apiParams.put("authorAccount", article.getId());
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        /*if (self.getAccount().equals(article.getTitle())) {
    ...
    public void onRightClick() {
        ...
        apiParams.put("gid", article.getId());
        HttpAPIRequester.execute(apiParams, URLConstant.ARTICLE_DELETE_URL);
        ArticleDBManager.getManager().deleteById(String.valueOf(article.getId()));
